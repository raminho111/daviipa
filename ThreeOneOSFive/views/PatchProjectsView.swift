...
private struct InjectFeaturesView: View {
    ...
    private enum Preset: CaseIterable, Identifiable {
        ...
        var symbol: String {
            ...
            case .saci: return "leaf"
            case .fps144: return "speedometer"
            case .neckLegit, .neckHigh, .legitMax, .umbigo, .headBodyMax, .altoNesquik, .barrigaNesquik, .pescocoNesquik: return "scope"
            }
        }

        var edition: GameEdition {
            switch self {
            case .headshot: return .max
            case .saci, .fps144: return .normal
            case .neckLegit, .neckHigh, .legitMax, .umbigo: return .max
            case .headBodyMax: return .normal
            case .altoNesquik: return .normal
            case .barrigaNesquik, .pescocoNesquik: return .max
            }
        }
    }
    ...
    private enum TexturePreset: CaseIterable, Identifiable {
        ...
        private static let replacementFilename = "optionalab_avatar_66.DfUs7MzeaoXWJ4jW"

        var id: UUID {
            let identifier: String
            switch self {
            case .mandela: identifier = "F3620E53-5B72-4734-BFF8-15A76B761601"
            case .apelapato: identifier = "0CB74A2A-4B87-4787-861A-5B8ED1D35802"
            case .ruok: identifier = "E51D169E-0967-40D5-ACB0-BBFBDE88E803"
            case .cria: identifier = "A177DFD5-31A6-4B17-9D16-963A80D2A104"
            }
        ...
        var title: String {
            switch self {
            case .mandela: return "MANDELA"
            case .apelapato: return "APELAPATO"
            case .ruok: return "RUOK"
            case .cria: return "CRIA"
            }
        }

        var edition: GameEdition { .normal }

        private var resourceName: String {
            switch self {
            case .mandela: return "mandela_avatar_66.DfUs7MzeaoXWJ4jW"
            case .apelapato: return "apelapato_avatar_66.DfUs7MzeaoXWJ4jW"
            case .ruok: return "rouk_avatar_66.DfUs7MzeaoXWJ4jW"
            case .cria: return "cria_avatar_66.DfUs7MzeaoXWJ4jW"
            }
        }
        ...
        func project() throws -> PatchProject {
            ...
            do {
                replacementData = try Data(contentsOf: url, options: .mappedIfSafe)
            } catch {
                throw TextureApplicationError.resourceUnavailable
            }
        ...
        private func verifyTargetExists() throws {
            guard let containerPath = ContainerStore.resolveAppContainerPath(bundleID: Self.targetBundleID),
                  ContainerStore.isApplicationContainerPath(containerPath) else {
                throw PatchPackageError.targetAppUnavailable(Self.targetBundleID)
            }
            ...
            guard targetExists else {
                ...
                let contentCacheURL = documentsURL.appendingPathComponent("contentcache", isDirectory: true)
                let optionalURL = contentCacheURL.appendingPathComponent("Optional", isDirectory: true)
                let iosURL = optionalURL.appendingPathComponent("ios", isDirectory: true)
                let bundlesURL = iosURL.appendingPathComponent("gameassetbundles", isDirectory: true)
                let compulsoryURL = contentCacheURL.appendingPathComponent("Compulsory", isDirectory: true)
                let compulsoryIOSURL = compulsoryURL.appendingPathComponent("ios", isDirectory: true)
                ...
                let directEntries = (try? fileManager.contentsOfDirectory(atPath: bundlesURL.path))?.sorted() ?? []
                let matchingEntries = directEntries.filter { entry in
                    let name = entry.lowercased()
                    return name.contains("optionalab") || name.contains("avatar_66")
                }
                // TEMPORARY RELEASE DIAGNOSTIC: retain these three messages
                // until texture-container validation on a device is complete.
                log(
                ...
                log("patch: texture gameassetbundles direct entries=\(directEntries)")
                log("patch: texture gameassetbundles similar entries=\(matchingEntries)")
                if let entries = directSimilarEntries(at: contentCacheURL) {
                    log("patch: texture layout contentcache entries=\(entries)")
                }
                if let entries = directSimilarEntries(at: optionalURL) {
                    log("patch: texture layout optional entries=\(entries)")
                }
                if let entries = directSimilarEntries(at: iosURL) {
                    log("patch: texture layout optional-ios entries=\(entries)")
                }
                if let entries = directSimilarEntries(at: compulsoryURL) {
                    log("patch: texture layout compulsory entries=\(entries)")
                }
                if let entries = directSimilarEntries(at: compulsoryIOSURL) {
                    log("patch: texture layout compulsory-ios entries=\(entries)")
                }

                let compulsoryBundlesURL = compulsoryIOSURL.appendingPathComponent("gameassetbundles", isDirectory: true)
                if fileManager.fileExists(atPath: compulsoryBundlesURL.path) {
                    let entries = (try? fileManager.contentsOfDirectory(atPath: compulsoryBundlesURL.path))?.sorted() ?? []
                    ...
                    log("patch: texture compulsory-gameassetbundles direct entries=\(entries)")
                    log("patch: texture compulsory-gameassetbundles similar entries=\(similarEntries)")

                    let avatarURL = compulsoryBundlesURL.appendingPathComponent("avatar", isDirectory: true)
                    let avatarExists = fileManager.fileExists(atPath: avatarURL.path)
                    log("patch: texture compulsory-gameassetbundles avatar exists=\(avatarExists)")
                    if avatarExists {
                        let avatarEntries = (try? fileManager.contentsOfDirectory(atPath: avatarURL.path))?.sorted() ?? []
                        let avatarSimilarEntries = avatarEntries.filter { entry in
                            let name = entry.lowercased()
                            return name.contains("optionalab")
                                || name.contains("avatar_66")
                                || name.contains("avatar")
                        ...
                        log("patch: texture compulsory-gameassetbundles avatar direct entries=\(avatarEntries)")
                        log("patch: texture compulsory-gameassetbundles avatar similar entries=\(avatarSimilarEntries)")
                    }
                }
    ...
    private enum TextureApplicationError: Error {
        ...
        var messageKey: String {
            switch self {
            case .resourceUnavailable: return "texture.error.resource_unavailable"
            case .targetMissing: return "texture.error.target_missing"
            case .targetUnavailable: return "texture.error.target_unavailable"
            }
        }
    }
    ...
    private enum FeatureState: Equatable {
        ...
        var isOn: Bool {
            ...
            if case .working(let isOn) = self { return isOn }
            return false
        }
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: AppTheme.sectionSpacing) {
                    ...
                    featureSection("FUNÇÕES DE AIMBOT") {
                        ForEach(Preset.allCases.filter { $0.edition == selectedEdition }) { preset in
                            presetRow(preset)
                        }
                    }

                    featureSection("FUNÇÕES HOLOGRAMA") {
                        unavailableRow(title: "HOLOGRAMA ARMAS", symbol: "cube.transparent")
                    }
            ...
            .animation(.easeInOut(duration: AppTheme.motionDuration), value: selectedEdition)
    ...
    private var editionPicker: some View {
        Picker("Edição do jogo", selection: $selectedEdition) {
            Text("FREE FIRE NORMAL").tag(GameEdition.normal)
            Text("FREE FIRE MAX").tag(GameEdition.max)
        }
    ...
    private func presetRow(_ preset: Preset) -> some View {
        ...
        return HStack(spacing: 12) {
            AppRowIcon(systemName: preset.symbol, tint: AppTheme.accent, frameSize: 34)
            VStack(alignment: .leading, spacing: 3) {
                titleWithEdition(preset.title, edition: preset.edition)
            ...
            Toggle("", isOn: toggleBinding(for: preset))
                .labelsHidden()
                .disabled(state.isWorking || isPatchOperationInFlight || store.isBusy)
    ...
    private func textureRow(_ texture: TexturePreset) -> some View {
        ...
        return HStack(spacing: 12) {
            AppRowIcon(systemName: "person.crop.square", tint: AppTheme.accent, frameSize: 34)
            VStack(alignment: .leading, spacing: 3) {
                titleWithEdition(texture.title, edition: texture.edition)
                statusText(for: state)
            }
            Spacer(minLength: 12)
            if state.isWorking {
            ...
            Toggle("", isOn: textureToggleBinding(for: texture))
                .labelsHidden()
                .disabled(state.isWorking || isPatchOperationInFlight || store.isBusy)
    ...
    private func titleWithEdition(_ title: String, edition: GameEdition) -> some View {
        HStack(spacing: 6) {
            Text(title)
            ...
            Text(edition.label)
                ...
                .tracking(0.35)
                .foregroundStyle(edition == .max ? AppTheme.accent : Color.secondary)
                .padding(.horizontal, 6)
                .padding(.vertical, 2)
                .background(
                    edition == .max ? AppTheme.accent.opacity(0.16) : Color.white.opacity(0.08),
    ...
    private func statusText(for state: FeatureState) -> some View {
        Group {
            switch state {
            case .unknown:
                Label("Estado não verificado", systemImage: "questionmark.circle")
            case .inactive:
                Text("Desativado")
            case .active:
                Text("Ativado nesta sessão")
            case .working(let isOn):
                Text(isOn ? "Aplicando…" : "Restaurando…")
            }
        }
        .font((state.isWorking || state.isOn) ? .caption.weight(.medium) : .caption)
        .foregroundStyle((state.isWorking || state.isOn) ? AppTheme.accent : .secondary)
    }

    private func unavailableRow(title: String, symbol: String) -> some View {
        HStack(spacing: 12) {
            AppRowIcon(systemName: symbol, tint: .secondary, frameSize: 34)
            VStack(alignment: .leading, spacing: 3) {
                Text(title)
                    .font(.body.weight(.semibold))
            ...
            Toggle("", isOn: .constant(false))
                .labelsHidden()
                .disabled(true)
                .accessibilityLabel("\(title), Em configuração")
        }
        .padding(.horizontal, 14)
        .padding(.vertical, 13)
        .opacity(0.78)
    ...
    private func toggleBinding(for preset: Preset) -> Binding<Bool> {
        Binding(
            get: { (states[preset.id] ?? .unknown).isOn },
            set: { requestedOn in
                performToggle(for: preset, requestedOn: requestedOn)
            }
        )
    }

    private func performToggle(for preset: Preset, requestedOn: Bool) {
        let current = states[preset.id] ?? .unknown
        guard !current.isWorking, !isPatchOperationInFlight, !store.isBusy else { return }

        if requestedOn {
            apply(preset)
        } else if case .active(let receipt) = current {
            restore(preset, receipt: receipt)
        }
    }

    private func textureToggleBinding(for texture: TexturePreset) -> Binding<Bool> {
        Binding(
            get: { (states[texture.id] ?? .unknown).isOn },
            set: { requestedOn in
                performTextureToggle(for: texture, requestedOn: requestedOn)
            }
        )
    }

    private func performTextureToggle(for texture: TexturePreset, requestedOn: Bool) {
        let current = states[texture.id] ?? .unknown
        guard !current.isWorking, !isPatchOperationInFlight, !store.isBusy else { return }

        if requestedOn {
            if let active = activeTexture(excluding: texture) {
                replaceTexture(active.texture, receipt: active.receipt, with: texture)
            } else {
                apply(texture)
            }
        } else if case .active(let receipt) = current {
            restore(texture, receipt: receipt)
        }
    }

    private func activeTexture(
        excluding requested: TexturePreset
    ) -> (texture: TexturePreset, receipt: PatchTransactionReceipt)? {
        for texture in TexturePreset.allCases where texture != requested {
            if case .active(let receipt) = states[texture.id] ?? .unknown {
    ...
    private func apply(_ preset: Preset) {
        guard let item = store.items.first(where: { $0.id == preset.id }) else {
        ...
        guard !item.isLocked, let project = item.project else {
            if item.isOfficialPreset {
                ...
                store.requestUnlock(for: item) { [self] packageID, success in
                    guard packageID == pendingAutoApplyFor,
                          let preset = Preset.allCases.first(where: { $0.id == packageID })
                    else {
                    ...
                    pendingAutoApplyFor = nil
                    if success,
                       let unlocked = store.items.first(where: { $0.id == preset.id }),
                       !unlocked.isLocked {
                        apply(preset)
                    }
                }
        ...
        states[preset.id] = .working(isOn: true)
        isPatchOperationInFlight = true
        Task.detached(priority: .userInitiated) {
            do {
                let receipt = try DevicePatchService.apply(project: project)
                await MainActor.run {
                    states[preset.id] = .active(receipt)
                    isPatchOperationInFlight = false
                }
    ...
    private func apply(_ texture: TexturePreset) {
        states[texture.id] = .working(isOn: true)
        isPatchOperationInFlight = true
        Task.detached(priority: .userInitiated) {
            do {
                let receipt = try DevicePatchService.apply(project: texture.project())
                await MainActor.run {
                    states[texture.id] = .active(receipt)
                    isPatchOperationInFlight = false
                }
                ...
                await MainActor.run {
                    ...
                    isPatchOperationInFlight = false
                    actionAlert = PatchStoreAlert(titleKey: "common.failed", messageKey: error.messageKey)
                }
            } catch let error as PatchPackageError {
                await MainActor.run {
                    states[texture.id] = .unknown
    ...
    private func replaceTexture(
        ...
        states[previous.id] = .working(isOn: false)
        states[next.id] = .working(isOn: true)
        isPatchOperationInFlight = true
        Task.detached(priority: .userInitiated) {
            ...
            do {
                ...
                let nextReceipt = try DevicePatchService.apply(project: next.project())
                await MainActor.run {
                    states[previous.id] = .unknown
                    states[next.id] = .active(nextReceipt)
                    isPatchOperationInFlight = false
                }
            } catch let error as TextureApplicationError {
                await MainActor.run {
                    states[previous.id] = restoredPrevious ? .unknown : .active(receipt)
                    states[next.id] = .unknown
                    isPatchOperationInFlight = false
                    actionAlert = PatchStoreAlert(titleKey: "common.failed", messageKey: error.messageKey)
                }
            } catch let error as PatchPackageError {
    ...
    private func restore(_ preset: Preset, receipt: PatchTransactionReceipt) {
        states[preset.id] = .working(isOn: false)
        isPatchOperationInFlight = true
        Task.detached(priority: .userInitiated) {
            do {
                try DevicePatchService.restore(receipt: receipt)
                await MainActor.run {
                    states[preset.id] = .inactive
                    isPatchOperationInFlight = false
                }
            } catch let error as PatchPackageError {
                await MainActor.run {
    ...
    private func restore(_ texture: TexturePreset, receipt: PatchTransactionReceipt) {
        states[texture.id] = .working(isOn: false)
        isPatchOperationInFlight = true
        Task.detached(priority: .userInitiated) {
            do {
                try DevicePatchService.restore(receipt: receipt)
    ...
    private func resetToUnknown() {
        for preset in Preset.allCases where states[preset.id] == nil {
            states[preset.id] = .unknown
        }
        for texture in TexturePreset.allCases where states[texture.id] == nil {
            states[texture.id] = .unknown
        }
    }
}
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                if isWorking {
                    ProgressView()
                } else if !isWorkspaceProject && !isOfficialProject {
