...
struct FileBrowserView: View {
    ...
    @State private var targetInformation: PatchTargetInformation?
    ...
    var body: some View {
        ...
        .sheet(item: $targetInformation) { information in
            PatchTargetInformationView(information: information)
        }
        .overlay {
            ZStack {
                if let activityText {
    ...
    private func fileActions(for entry: FileEntry) -> some View {
        ...
        if !entry.isDirectory {
            if let information = targetInformation(for: entry) {
                Button {
                    targetInformation = information
                } label: {
                    Label(
                        language.text("browser.target_information"),
    ...
    private func targetInformation(for entry: FileEntry) -> PatchTargetInformation? {
        guard !entry.isDirectory, let bundleID else { return nil }
        let root = URL(fileURLWithPath: containerPath, isDirectory: true).standardizedFileURL
        let target = URL(fileURLWithPath: entry.path).standardizedFileURL
        guard target.path.hasPrefix(root.path + "/") else { return nil }

        let relativePath = String(target.path.dropFirst(root.path.count + 1))
        guard let canonicalRelativePath = try? PatchPathValidator.canonicalRelativePath(relativePath) else {
            return nil
        }
        return PatchTargetInformation(
            bundleID: bundleID,
...
private struct PatchTargetInformation: Identifiable {
    let id = UUID()
    let bundleID: String
...
private struct PatchTargetInformationView: View {
    @Environment(\.appLanguage) private var language
    @Environment(\.dismiss) private var dismiss
    let information: PatchTargetInformation
    ...
    var body: some View {
        NavigationStack {
            List {
                Section {
                    detail(language.text("browser.target_bundle_id"), value: information.bundleID)
                    detail(language.text("browser.target_relative_path"), value: information.relativePath)
                    detail(language.text("browser.target_filename"), value: information.filename)
                    detail(
                        language.text("browser.target_size"),
                        value: ByteCountFormatter.string(
                            fromByteCount: information.byteCount,
                            countStyle: .file
                        ) + " (\(information.byteCount) bytes)"
                    )
                } footer: {
                    Text(language.text("browser.target_information_footer"))
                }
            }
            .navigationTitle(language.text("browser.target_information"))
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button {
                        UIPasteboard.general.string = information.clipboardText
                        copied = true
                    } label: {
                        Label(
                            language.text(copied ? "browser.target_copied" : "browser.target_copy"),
                            systemImage: copied ? "checkmark" : "doc.on.doc"
                        )
                    }
                    .accessibilityLabel(language.text("browser.target_copy"))
                }
            }
        }
    }
